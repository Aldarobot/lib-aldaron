...
static inline void jlgr_vo_bounding_box(la_window_t* jlgr, la_vo_t* vo,
...
static void jlgr_vo_poly__(la_window_t* jlgr, la_vo_t* vo, uint32_t vertices,
	const float *xyzw)
{
	const float FS_RECT[] = {
		0.,jl_gl_ar(jlgr),0.,
...
void jlgr_vo_init__(la_window_t* window, la_vo_t* vo) {
...
void jlgr_vo_exit__(la_vo_t* vo, const char* error) {
...
void la_vo_pr_draw(la_vo_t* vo, uint8_t orient) {
	...
	jlgr_pr_draw(vo->window, &vo->pr, vo->pr.cb.pos, orient);
}
